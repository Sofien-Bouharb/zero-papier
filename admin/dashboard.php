...
<?php if ($view === 'boards'): ?>
  ...
  <a href="#" class="btn btn-success m-3 ">📤 Ajouter un code index</a>
...
<?php elseif ($view === 'posts'): ?>
  ...
  <a href="#" class="btn btn-success m-3">📤 Ajouter un poste</a>
  ...
  <table class="table table-dark table-bordered table-hover">
    ...
    <tbody>
      <?php
      $workers = $pdo->query("SELECT step_number,hostname, ip_address FROM documents_search.workers ORDER BY hostname")->fetchAll();
    ...
    <tbody>
      ...
      foreach ($rows as $row): ?>
        <tr>
          ...
          <td><span class="text-info"><?= htmlspecialchars($row['hostname']) ?></span></td>
