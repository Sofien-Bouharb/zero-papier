...
// Récupération des postes et cartes
$workers = $pdo->query("SELECT step_number, hostname FROM documents_search.workers ORDER BY hostname")->fetchAll();
    ...
    <div class="mb-3">
      ...
      <select id="selected_post" class="form-select">
        ...
        <?php foreach ($workers as $w): ?>
          <option value="<?= $w['step_number'] ?>"><?= htmlspecialchars($w['hostname']) ?></option>
        <?php endforeach; ?>
      </select>
    </div>
...
function addMapping() {
  ...
  mappings.push({ step_number: postId, board_ids: boardIds });
